var microast = {
  Struct: (function() {
    ...
    Struct.prototype.generate = function() {
      console.log("Generating Struct " + this.type());
      var res = [
        "struct " + this.type() + " {",
    ...
    Struct.prototype.generateMakerFunction = function () {
      ...
      return [
        "function _make" + this.name + "(json:JSON) : " + this.type() + " {",
  ...
  List: (function() {
    ...
    List.prototype.generateJsonConverter = function(fieldname) {
      return "_make" + this.ofNode.type() + '(json.get("' + fieldname + '"))';
    };
    List.prototype.generateMakerFunction = function() {
      ...
      if (microast.simpleTypes.indexOf(this.ofNode.name) > -1) {
        ...
        asType = "_make" + this.ofNode.name + "(json)";
      } else if (this.ofNode instanceof microast.List) {
        asType = "_make" + this.ofNode.name + "(json.asList())";
      } else {
        throw new Error("Unknown type " + this.ofNode.name);
      }
      return [
        "function _make" + this.name + "(jsonList:list<JSON>) : " + this.type() + " {",
