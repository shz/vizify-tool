...
var websocketHandler = function(app, ws) {
  ...
  var onClose = function() {
    ...
    app.locals.state.removeListener('vizData', onUpdateVizData);
  };
  var onUpdateVizData = function(data) {
    if (watching) {
      ws.send(JSON.stringify({
        type: 'vizData',
        data: data
  ...
  app.locals.state.on('vizData', onUpdateVizData);
  ws.once('close', onClose);

  // Bootstrap!
  console.log('Client connected');
  ws.emit('message', JSON.stringify({type: 'watch'}));
  app.locals.state.updateVizData();
