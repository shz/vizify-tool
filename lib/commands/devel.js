...
var handleStaticRequest = function(dir, req, res) {
  ...
  if (filename == '/dev/styl/main.css') {
    filename = '/dev/styl/main.styl';
  }
      ...
      fs.readFile(fullpath, function(err, data) {
        if (err) {
          ...
          if (filename.match(/\.styl$/)) {
            stylus.render(data.toString('utf8'), {
              filename: filename,
              paths: [path.join(__dirname, '..', '..', 'static', 'dev', 'styl')]
            }, function(err, css) {
              if (err) {
                res.writeHead(500, {'Content-Type': 'text/plain; charset=utf-8'});
                res.end(err.stack);
    ...
    if (cache) {
      ...
      card.create({
        ...
        if (err) {
          ...
          if (err.src && err.filename && err.loc) {
