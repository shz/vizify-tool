...
var usage = 'Usage: vz compile javascript|cpp [--libs LIB_PATH] [-o|--ouput OUTPUT_DIR] [-p|--prune ENTRYPOINT] [--browserify] [--no-demo]';
...
module.exports = function(options) {
  ...
  options = {
    ...
    demo: (options.demo === undefined) || options.demo
  };

  options.prune = (function(p, prune) {
    if (!p && !prune) {
      return null;
    }
    var entryPoints = [];
    if (p) { entryPoints = entryPoints.concat(p); }
    if (prune) { entryPoints = entryPoints.concat(prune); }

    return {entryPoints: entryPoints};
  }(options.p, options.prune));
