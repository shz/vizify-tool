...
module.exports = function(options) {
  ...
  card.create({
    ...
    checkExisting('card.js', function(err, cardExists) {
      if (err) {
        console.error('Failed to check for existing card at', folder + filename);
        process.exit(1);
      }
      ...
      if (process.stdin.isTTY) {
        prompt("A version of this card has already been published; type 'yes' to overwrite.", 'no', function(err, response) {
