...
var prodHost = 'yahoo-cards.global.ssl.fastly.net';
var devHost = 'cards-data.rc.staging.manhattan.gq1.yahoo.com';

exports.getDataUrls = function(card, mode, datahost) {
  // caller can override built-in prod/staging host by passing in non-null datahost param
  var host = datahost ? datahost : (mode === 'production' ? prodHost : devHost);
  var root = (mode === 'production' ? 'https' : 'http')  +
             '://' +
             host +
             '/data/live/';
  var results = [];
