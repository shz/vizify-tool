  ...
  async.waterfall([
    ...
    function(card, callback) {
      ...
      if (options.params['datafile'] && options.params['datafile'] != "none") {
        dataSource = path.join('testdata', options.params['datafile']);
      }
      if (options.mode == 'development') {
        // grab test data files
        try {
          var files = fs.readdirSync(path.join(options.dir, 'testdata'));
          testDataFiles = files.map(function(f) {
            return {name: f, selected: f === options.params['datafile']};
          });
        }
        catch(e) {
          // only complain if user is requesting a datafile.
          if (options.params['datafile']) {
            console.log("Error reading testdata directory. Does it exist?");
          }
        }

        // if we haven't overridden dataSource with a local file...
        if (!dataSource) {
